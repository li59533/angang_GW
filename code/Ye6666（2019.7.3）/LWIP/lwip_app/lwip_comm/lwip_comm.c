...
void lwip_comm_default_ip_set(__lwip_dev *lwipx)
{
	...
	*/
	...
	lwipx->ip[0]=sys_para.ip[0];	
	lwipx->ip[1]=sys_para.ip[1];	
	lwipx->ip[2]=sys_para.ip[2];	
	lwipx->ip[3]=sys_para.ip[3];	
	//默认子网掩码:255.255.255.0
	lwipx->netmask[0]=sys_para.netmask[0];
	lwipx->netmask[1]=sys_para.netmask[1];
	lwipx->netmask[2]=sys_para.netmask[2];
	lwipx->netmask[3]=sys_para.netmask[3];
	//默认网关:192.168.1.1
	lwipx->gateway[0]=sys_para.gateway[0];
	lwipx->gateway[1]=sys_para.gateway[1];
	lwipx->gateway[2]=sys_para.gateway[2];
	lwipx->gateway[3]=sys_para.gateway[3];	
...
u8  lwip_comm_init(void)
{
	...
	{//使用静态IP
		inet_aton(config.LocalIP,&ipaddr);
		inet_aton(config.LocalGATEWAY,&gw);
		inet_aton(config.LocalMASK,&netmask);
	}	
